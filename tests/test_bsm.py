...
class TestBlackScholesModel(unittest.TestCase):
    ...
    def test_put_option_price(self):
        ...
        self.assertAlmostEqual(
            price, expected_price, places=4, msg="Put option price mismatch"
        )
    ...
    def test_put_call_parity(self):
        ...
        call_price, _, _, _, _, _, _, _, _, _ = black_scholes_greeks(
            "C", S, K, T, r, sigma
        )
        put_price, _, _, _, _, _, _, _, _, _ = black_scholes_greeks(
            "P", S, K, T, r, sigma
        )
    ...
    def test_greeks_call_delta(self):
        ...
        self.assertAlmostEqual(
            delta, expected_delta, places=4, msg="Call Delta mismatch"
        )
    ...
    def test_greeks_put_delta(self):
        ...
        self.assertAlmostEqual(
            delta, expected_delta, places=4, msg="Put Delta mismatch"
        )

    def test_greeks_gamma(self):
        S, K, T, r, sigma = 100, 100, 1.0, 0.05, 0.20
        expected_gamma = 0.0187
        _, _, _, _, _, _, gamma_c, _, _, _ = black_scholes_greeks(
            "C", S, K, T, r, sigma
        )
        _, _, _, _, _, _, gamma_p, _, _, _ = black_scholes_greeks(
            "P", S, K, T, r, sigma
        )
        # Changer places=4 à places=3
        self.assertAlmostEqual(
            gamma_c, expected_gamma, places=3, msg="Call Gamma mismatch"
        )
        self.assertAlmostEqual(
            gamma_p, expected_gamma, places=3, msg="Put Gamma mismatch"
        )
    ...
    def test_greeks_theta_call(self):
        ...
        self.assertAlmostEqual(
            theta, expected_theta_per_day, places=3, msg="Call Theta mismatch"
        )
    ...
    def test_greeks_theta_put(self):
        ...
        self.assertAlmostEqual(
            theta, expected_theta_per_day, places=3, msg="Put Theta mismatch"
        )
    ...
    def test_greeks_rho_call(self):
        ...
        self.assertAlmostEqual(
            rho, expected_rho_per_percent, places=3, msg="Call Rho mismatch"
        )
    ...
    def test_greeks_rho_put(self):
        ...
        self.assertAlmostEqual(
            rho, expected_rho_per_percent, places=3, msg="Put Rho mismatch"
        )
    ...
    def test_T_zero(self):
        ...
        (
            ...
            theta_c,
            rho_c,
        ) = black_scholes_greeks("C", S, K, T, r, sigma)
        ...
        (
            ...
            theta_p,
            rho_p,
        ) = black_scholes_greeks("P", S, K, T, r, sigma)
    ...
    def test_sigma_zero(self):
        ...
        self.assertAlmostEqual(
            price_c, max(0, S * np.exp(r * T) - K) * np.exp(-r * T), places=4
        )
        self.assertEqual(gamma_c, 0.0)
        self.assertEqual(vega_c, 0.0)
        self.assertEqual(theta_c, 0.0)
        ...
        self.assertAlmostEqual(
            price_p, max(0, K - S * np.exp(r * T)) * np.exp(-r * T), places=4
        )
        self.assertEqual(gamma_p, 0.0)
        self.assertEqual(vega_p, 0.0)
        self.assertEqual(theta_p, 0.0)
