...
from core.monte_carlo_pricer import run_monte_carlo # Assurez-vous que core est aussi sous src si c'était votre structure
...
class TestExoticOptionPricing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.N_simulations = 500000 # Augmenter pour une meilleure précision Monte Carlo
    ...
    def test_price_heston_asian_option_benchmark(self):
        ...
        price = price_heston_asian_option(
            S0=self.S0, V0=self.V0, kappa=self.kappa, theta=self.theta, xi=self.xi, rho=self.rho,
            T=self.T, r=self.r, N_steps=self.N_steps, N_simulations=self.N_simulations,
            K=50.0, option_type='C'
        )
        ...
        self.assertAlmostEqual(price, expected_price, delta=delta, 
