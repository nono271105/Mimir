...
from src.models.heston.heston_model import heston_price # Pour obtenir un prix de référence analytique
...
class TestMonteCarloPricer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.N_simulations = 100000 # Nombre de simulations élevé pour une meilleure convergence MC
    ...
    def test_european_call_pricing(self):
        ...
        paths_S, _ = generate_heston_paths(
            self.S0, self.V0, self.kappa, self.theta, self.xi, self.rho,
            self.T, self.r, self.N_steps, self.N_simulations
        )
        ...
        ref_price_call = heston_price(
            S=self.S0, K=self.K, T=self.T, r=self.r,
            kappa=self.kappa, theta=self.theta, sigma=self.xi, # 'sigma' prend la valeur de 'xi'
            rho=self.rho, v0=self.V0, option_type='C'
        )
        print(f"Prix Call Heston Analytique: {ref_price_call:.4f}")

        self.assertAlmostEqual(mc_price_call, ref_price_call, delta=0.5,
    ...
    def test_european_put_pricing(self):
        ...
        ref_price_put = heston_price(
            S=self.S0, K=self.K, T=self.T, r=self.r,
            kappa=self.kappa, theta=self.theta, sigma=self.xi, # 'sigma' prend la valeur de 'xi'
            rho=self.rho, v0=self.V0, option_type='P'
        )
        print(f"Prix Put Heston Analytique: {ref_price_put:.4f}")

        self.assertAlmostEqual(mc_price_put, ref_price_put, delta=0.5,
