...
class TestBinomialModel(unittest.TestCase):
    ...
    def test_call_option_price(self):
        ...
        self.assertAlmostEqual(
            price, expected_price, places=4, msg="Binomial Call option price mismatch"
        )
    ...
    def test_put_option_price(self):
        ...
        self.assertAlmostEqual(
            price, expected_price, places=4, msg="Binomial Put option price mismatch"
        )
    ...
    def test_N_one(self):
        ...
        # Prix Call à t=0
        expected_call_price = np.exp(-r * dt) * (
            q * call_payoff_u + (1 - q) * call_payoff_d
        )
        ...
        # Prix Put à t=0
        expected_put_price = np.exp(-r * dt) * (
            q * put_payoff_u + (1 - q) * put_payoff_d
        )

        calculated_call_price = binomial_option_pricing(
            "C", S, K, T, r, sigma, N, exercise_type="EU"
        )
        calculated_put_price = binomial_option_pricing(
            "P", S, K, T, r, sigma, N, exercise_type="EU"
        )

        self.assertAlmostEqual(
            calculated_call_price,
            expected_call_price,
            places=4,
            msg="Binomial Call N=1 mismatch",
        )
        self.assertAlmostEqual(
            calculated_put_price,
            expected_put_price,
            places=4,
            msg="Binomial Put N=1 mismatch",
        )
    ...
    def test_american_call_price(self):
        ...
        S, K, T, r, sigma, N = (
            ...
            0.20,
            500,
        )  # N élevé pour meilleure approximation

        # Prix Européen (BSM pour référence, ou notre binomial EU)
        bsm_price_eu, _, _, _, _, _, _, _, _, _ = black_scholes_greeks(
            "C", S, K, T, r, sigma
        )
        binomial_price_eu = binomial_option_pricing(
            "C", S, K, T, r, sigma, N, exercise_type="EU"
        )

        # Prix Américain
        binomial_price_us = binomial_option_pricing(
            "C", S, K, T, r, sigma, N, exercise_type="US"
        )
        ...
        self.assertAlmostEqual(
            ...
            binomial_price_eu,
            places=4,
            msg="American Call (no dividend) should equal European Call",
        )
        self.assertAlmostEqual(
            binomial_price_us,
            bsm_price_eu,
            places=2,
            msg="American Call should approximate BSM Call",
        )
    ...
    def test_american_put_price(self):
        ...
        # Prix Américain
        binomial_price_us = binomial_option_pricing(
            "P", S, K, T, r, sigma, N, exercise_type="US"
        )
        ...
        self.assertGreaterEqual(
            binomial_price_us,
            binomial_price_eu,
            msg="American Put price should be >= European Put price",
        )
        expected_american_put_price = 6.088810110703
        self.assertAlmostEqual(
            binomial_price_us,
            expected_american_put_price,
            places=4,
            msg="American Put price mismatch with expected value",
        )
    ...
    def test_american_put_deep_in_the_money(self):
        ...
        self.assertGreater(
            binomial_price_us,
            binomial_price_eu,
            msg="Deep ITM American Put price should be significantly > European Put price",
        )
        ...
        self.assertGreaterEqual(
            binomial_price_us,
            K - S,
            msg="American Put should be at least its intrinsic value",
        )
