...
@njit
def _heston_char_func_components_numba(
    phi: float,
    T: float,
    r: float,
    kappa: float,
    ...
    if (
        abs(b_term + d_val) < 1e-18
    ):  # Ajout d'une petite tolérance pour éviter division par zéro
    ...
    if abs(log_arg_denom) < 1e-18:  # Éviter log(0)
        log_term = (
            0 + 0j
        )  # Gérer comme une valeur nulle si le dénominateur est quasi-nul
    else:
...
def _heston_integral_numba(
    ...
    for i in prange(len(phi_values)):
        ...
        C, D = _heston_char_func_components_numba(
            phi, T, r, kappa, theta, sigma, rho, u_val
        )
        ...
        integrand_term = (
            cmath.exp(-(0 + 1j) * phi * cmath.log(K)) * f_j_S / ((0 + 1j) * phi)
        )
...
def heston_price(
    ...
    phi_values = np.linspace(
        1e-10, upper_limit, num_points
    )  # Commence à un petit epsilon

    # Calcul de P1
    integral_P1_complex = _heston_integral_numba(
        phi_values, S, K, T, r, kappa, theta, sigma, rho, v0, u_val=0.5
    )
    P1 = 0.5 + (
        integral_P1_complex.real / np.pi
    )  # On prend la partie réelle de l'intégrale

    # Calcul de P2
    integral_P2_complex = _heston_integral_numba(
        phi_values, S, K, T, r, kappa, theta, sigma, rho, v0, u_val=-0.5
    )
    P2 = 0.5 + (
        integral_P2_complex.real / np.pi
    )  # On prend la partie réelle de l'intégrale
