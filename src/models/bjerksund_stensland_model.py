...
def _black_scholes_greeks_internal(
    ...
    if option_type == "C":
        ...
        theta = (-(S * np.exp(-q * T) * n_d1 * sigma) / (2 * sqrt_T)
                 - r * K * np.exp(-r * T) * N_d2
                 + q * S * np.exp(-q * T) * N_d1)
...
def newton_raphson_bs_american_call(
    S_current: float, K: float, T: float, r: float, sigma: float, q: float, epsilon: float = 1e-6
) -> float:
    """
...
def newton_raphson_bs_american_put(
    ...
    for _ in range(200): # Augmenter les itérations
        ...
        if abs(F_prime_S_star) < 1e-10:
            S_star_new = S_star_old - 0.001 * K # Petit pas si la dérivée est proche de zéro
        else:
            S_star_new = S_star_old - F_S_star / F_prime_S_star
...
def bjerksund_stensland_2002(
    ...
    beta_val = (0.5 - b / sigma_squared) + np.sqrt(((b / sigma_squared) - 0.5)**2 + 2 * r / sigma_squared)
    ...
    if option_type == 'C':
        ...
        A2_term = (K * np.exp(-r * T) * st.norm.cdf(-y2) - S * np.exp(-q * T) * st.norm.cdf(-y1))
        ...
        A2_prime_term = (K * np.exp(-r * T) * st.norm.cdf(-y2) - S * np.exp(-q * T) * st.norm.cdf(-y1))
