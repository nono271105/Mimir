...
def binomial_option_pricing(option_type: str, S: float, K: float, T: float, r: float, sigma: float, N: int, exercise_type: str = 'EU', dividend_yield: float = 0.0, dividend_amount: float = 0.0, dividend_time: float = -1.0):
    """
    ...
    for i in range(N + 1): # i représente le numéro de pas (de 0 à N)
        for j in range(i + 1): # j représente le nombre de mouvements "vers le haut" (de 0 à i)
    ...
    if dividend_amount > 0 and 0 < dividend_time < T:
        ...
        for i in range(div_step, N + 1): # À partir du pas du dividende jusqu'à l'échéance
            for j in range(i + 1):
    ...
    for j in range(N + 1):
        if option_type == 'C':
            ...
            raise ValueError("Type d'option invalide. Utilisez 'C' pour Call ou 'P' pour Put.")
    ...
    for i in range(N - 1, -1, -1): # De N-1 jusqu'à 0
        for j in range(i + 1): # Pour chaque nœud à ce pas
            ...
            continuation_value = (p * option_values[i + 1, j + 1] + q * option_values[i + 1, j]) * np.exp(-r * dt)
