...
def display_bsm_results(option_type: str, option_price: float, d1: float, d2: float, N_d1: float, N_d2: float, 
...
def plot_payoff(option_type: str, S: float, K: float, option_price: float):
    ...
    plt.plot(S_payoff_range, profit_loss_values, label=f"Profit/Perte de l'option {option_type}", color='purple')
    ...
    plt.title(f"Profit/Perte net de l'option {option_type} à l'échéance (Prix de l'option = {option_price:.2f})")
